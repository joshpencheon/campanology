...
puts "  - & #{weighted_graph.edges.length} adjs"
puts "  - weights: #{weighted_graph.edges.map {|e| e[1] }.uniq.inspect}"
...
puts "  hamiltonian cycle already?: #{ExtentChecker.new(mst.nodes.map(&:label)).check}"
...
# odd_nodes = mst.nodes.select { |node| node.degree % 2 == 1  }
...
multigraph.nodes.each do |node|
  puts "#{node} -> #{node.degree} --- #{node.connected_nodes}"
  puts "MULTI" if node.connected_nodes.length != node.degree
end
...
# puts multigraph.edges.select { |edge| multigraph.edges.count(edge) > 1 }.inspect

eulerian_circuit = CircuitFinder.new(multigraph).seek!

# puts eulerian_circuit.uniq.sort == multigraph.nodes.sort
# puts (multigraph.nodes - eulerian_circuit.uniq).inspect

puts "#{eulerian_circuit.length - 1}, #{multigraph.edges.length}"
puts eulerian_circuit.inject({}) {|hash, vertex| count = eulerian_circuit.count(vertex); hash[count] ||= []; hash[count] << vertex; hash }.map {|k, v| {k => v.length} }.inspect
